...
class Server(BasicServer):
    ...
    def iterate(self):
        ...
        #     aaa = sum([np.prod(list(p.size())) for p in models[i].parameters()])
        #     para += aaa
        # print('params: {:4f}M'.format(para * 4 / 1000 / 1000))
        # print("+++++++++++++++++++++++++++++++++++++++")

        # aggregate: pk = 1/K as default where K=len(selected_clients)
        self.model[0] = self.aggregate(models)
        ws = copy.deepcopy(self.model[0])
        self.model = [ws for i in range(10)]
