...
import collections
import torch
from my_utils.myfflow import powerlaw
import copy
...
class POW(BasicPartitioner):
    ...
    def __call__(self, data):
        ...
        a = 0
        for i in range(10):
            a = a + len(local_datas[i])
            local_datas[i] = data.indices[a-len(local_datas[i]):a]
        return local_datas


class CLA(BasicPartitioner):
    def __init__(self, num_clients=100):
        self.num_clients = num_clients
    ...
    def __call__(self, data):
        n_classes = 10
        sample_size_cap = int(self.num_clients) * 2000
        attrs = [d[-1] for d in data]
        data_indices = [torch.nonzero(torch.tensor(attrs) == class_id).view(-1).tolist() for class_id in range(n_classes)]
        a = copy.deepcopy(data_indices)
        for i in range(len(data_indices)):
            for j in range(len(data_indices[i])):
                data_indices[i][j] = data.indices[a[i][j]]
